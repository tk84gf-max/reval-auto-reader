...
function buildInput(f, actual) {
  ...
  const input = {
    property: { name: f.name || '物件', price: f.price, grossYield: gy, buildAge: f.buildAge, struct: f.struct || 'rc', floorArea: f.floorArea, landArea: f.landArea, rooms: f.rooms, address: f.address || '', use: f.useType || '' },
